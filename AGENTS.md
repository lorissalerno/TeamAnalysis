...
- **GitHub:** repo pubblico preconfigurato `lorissalerno/TeamAnalysis` in `js/version.js` (`DEFAULT_REPO`) — il controllo aggiornamenti funziona senza input utente. Mantenere `version.json`/`changelog.json` committati e pushati su `main` affinché `raw.githubusercontent.com` rifletta la versione corrente.
